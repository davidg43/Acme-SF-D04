...
public interface DeveloperDashboardRepository extends AbstractRepository {
	...
	@Query("SELECT count(t) FROM TrainingModule t WHERE t.developer.userAccount.id = :id and t.updateMoment IS NOT NULL and t.updateMoment NOT LIKE ''")
	int totalTrainingModulesWithUpdateMoment(int id);

	@Query("SELECT count(ts) FROM TrainingSession ts WHERE ts.trainingModule.developer.userAccount.id = :id and ts.link IS NOT NULL and ts.link NOT LIKE ''")
